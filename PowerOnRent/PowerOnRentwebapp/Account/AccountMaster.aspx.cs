...
using Obout.Grid;
using PowerOnRentwebapp.PopupMessages;
    ...
    public partial class AccountMaster : System.Web.UI.Page
    {
        ...
        protected void ResetUserControl()
        {
            try
            {
                ...
                //UC_StatutoryDetails1.BindGridStatutoryDetails(0, "Student", profile.Personal.CompanyID);
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            imgsearchRequest.Visible = false;
        ...
        private void BindSFTPGrid()
        {
            try
            {
                ...
                ds = CompanyClient.BindSFTPGrid(profile.DBConnection._constr);
                Gridsftp.DataSource = ds;
                Gridsftp.DataBind();
            }
            catch (System.Exception ex)
            {
                Login.Profile.ErrorHandling(ex, this, "Account Master", "BindSFTPGrid");
            }
        }
        ...
        private void loadstring()
        {
            try
            {
               ...
               // lbladdresstype.Text = rm.GetString("AddressType", ci);
                ...
                lbllocation.Text = rm.GetString("LocationCompulsory", ci);
                lblsourceEmail.Text = rm.GetString("SourceEmail", ci);

                lblTime.Text = rm.GetString("EcommerceTime", ci);
                lblRequestor.Text = rm.GetString("EcommerceRequestor", ci);
                lblmerchant.Text = rm.GetString("MerchantCode", ci);
                lblnormalOrder.Text = rm.GetString("NormalOrder", ci);
                lblpreorder.Text = rm.GetString("PreOrder", ci);
                lblcustanalytics.Text = rm.GetString("CustomerAnalytics", ci);
       ...
       #region Customer form code for GWC
            ...
            CompanySetupService.iCompanySetupClient CompanyClient = new CompanySetupService.iCompanySetupClient();
              ...
              try
              {
                  if (checkDuplicate() == "")
                  {
                      ...
                      objcompany.SourceEmail = txtsourceEmail.Text;
        ...
        public void fillCompany(long ID)
        {
            ...
            CompanySetupService.iCompanySetupClient company = new CompanySetupService.iCompanySetupClient();
            CustomProfile profile = CustomProfile.GetProfile();
        ...
        protected void btnadd_Click(object sender, System.EventArgs e)
        {
            ...
            string LocationComp = "", Customeranaly = ""; ;
            bool gwcdelivery = false, ecommerce = false, pricechange = false, chkmny = false, ERPAutoapprove = false; 
            ...
            try
            {
                ...
                long RequestorID = 0;
                int MerchantCode = 0;
                decimal TokenExptime = 0;
                decimal TokenpreOrdertime = 0;
                ...
                bool locCompulsory = radiolocationNo.Checked;
                if (locCompulsory == true)
                {
                    LocationComp = "No";
                }
                else
                ...
                bool custanalytics = radiocustanalycsno.Checked;
                if (custanalytics == true)
                {
                    Customeranaly = "No";
                }
                else
                ...
                if (chkecommerce.Checked == true)
                {
                   ...
                   // string deptid = hdnSelectedDepartment.Value;
                   check = company.checkmerchantcode(deptid, profile.DBConnection._constr);
                    if(check== 0)
                    {
                        MerchantCode = GenerateRandomNo();
                    }
                    else
                    {
                        MerchantCode = int.Parse(check.ToString());
                    }

                    //if (hdnmerchantCode.Value == "")
                    //{
                    //    MerchantCode = GenerateRandomNo();
                    //}
                    //else
                    //{
                    //    MerchantCode = int.Parse(hdnmerchantCode.Value);
                    //}


                    RequestorID = long.Parse(hdnRequestorID.Value);
                    TokenExptime = decimal.Parse(txtelapseTime.Text);
                    TokenpreOrdertime = decimal.Parse(txtPreorderTime.Text);

                }
                if(chkemoney.Checked == true)
                {
                    chkmny = true;
                }

                if (chkautoapprove.Checked == true)
                {
                    ERPAutoapprove = true;
                }
                ...
                // start New Code for 2022 CR Project 3 Expiry 
                int ExpiryDays = 0;
                if (txtexpirydays.Text != "")
                {
                    ExpiryDays = int.Parse(txtexpirydays.Text.ToString());
                }
                // end New Code for 2022 CR Project 3 Expiry 
                ...
                if (editvar == "Edit")
                {
                    ...
                    if (chkecommerce.Checked == true)
                    {
                        ...
                        //if (dupliecommerce <= 0)
                        //{
                        company.UpdateDeptInfo(DeptId, deptname, deptcode, approval, autocancel, canceldays, userid, DateTime.Now, active, Approreminder, approschedays, autocancerem, autocanceschedays, gwcdelivery, ecommerce, OrderFormat, maxdeliveydays, Finapprover, addressType, pricechange, LocationComp,MerchantCode, TokenExptime, RequestorID,TokenpreOrdertime,Customeranaly,chkmny, ERPAutoapprove, ExpiryDays, profile.DBConnection._constr);
                        //}
                        //else
                        //{
                ...
                else
                {
                    ...
                    if (resultval <= 0)
                    {
                        if (chkecommerce.Checked == true)
                        {
                            ...
                            //if (dupliecommerce <= 0)
                            //{
                            company.Savecustdeptinfo(companyid, deptname, 1, deptcode, approval, autocancel, canceldays, userid, DateTime.Now, active, Approreminder, approschedays, autocancerem, autocanceschedays, gwcdelivery, ecommerce, OrderFormat, maxdeliveydays, addressType, pricechange, LocationComp, MerchantCode, TokenExptime, RequestorID,TokenpreOrdertime,Customeranaly, chkmny, ERPAutoapprove, ExpiryDays, profile.DBConnection._constr);
                            //}
                            //else
                            //{
        ...
        protected void imgBtnEditbom_OnClick(object sender, ImageClickEventArgs e)
        {
            ...
            try
            {
                ...
                //    lblmerchantCode.Visible = false;
                //    chkecommerce.Enabled = false;
                //}
                //else
                //{

                //    lblmerchantCode.Visible = true;
                ...
                if (dt.Rows.Count > 0)
                {
                    if (long.Parse(dt.Rows[0]["UserID"].ToString()) != 0)
                    {
                        lblmerchantCode.Text = dt.Rows[0]["MerchantCode"].ToString();
                        lblmerchant.Visible = true;
                        lblmerchantCode.Visible = true;
                        lblmerchant.Text = "Merchant Code";
                        chkecommerce.Enabled = true;
                    }
                    else
                    {
        ...
        //    CustomProfile profile = CustomProfile.GetProfile();
        //    CompanySetupService.iCompanySetupClient company = new CompanySetupService.iCompanySetupClient();
        //    ds = company.GetAddressType(profile.DBConnection._constr);
        ...
        //    ddladdressType.Items.Insert(0, lst);
        //}
        ...
        public void Getpettycash()
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows[0]["location"].ToString() == "No" || dt.Rows[0]["location"].ToString() == "")
                    {
                        radiolocationNo.Checked = true;
                        radiolocationYes.Checked = false;
                    }
                    else
                    {
                        radiolocationYes.Checked = true;
                        radiolocationNo.Checked = false;
                    }

                    if (dt.Rows[0]["CustAnalytics"].ToString() == "No" || dt.Rows[0]["CustAnalytics"].ToString() == "")
                    {
                        radiocustanalycsno.Checked = true;
                        radioCustanalyticsyes.Checked = false;
                    }
                    else
                    {
                        radioCustanalyticsyes.Checked = true;
                        radiocustanalycsno.Checked = false;
                    }
                    ...
                    if (dt.Rows[0]["ECommerce"].ToString() == "True")
                    {
                        ...
                        txtelapseTime.Enabled = true;
                        txtPreorderTime.Enabled = true;
                    }
                    if (long.Parse(dt.Rows[0]["UserID"].ToString()) != 0)
                    {
                        txtelapseTime.Text = dt.Rows[0]["TokenExptime"].ToString();
                        txtPreorderTime.Text = dt.Rows[0]["PreOrderExpTime"].ToString();
                        RequestorID = long.Parse(dt.Rows[0]["UserID"].ToString());
                        hdnRequestorID.Value = dt.Rows[0]["UserID"].ToString();
                        hdnmerchantCode.Value = dt.Rows[0]["MerchantCode"].ToString();
                        lblmerchantCode.Text = dt.Rows[0]["MerchantCode"].ToString();
                    }
                    txtecomReq.Text = company.getFinApprovername(RequestorID, profile.DBConnection._constr);
                    ...
                    if (dt.Rows[0]["Emoney"].ToString() == "True")
                    {
                        chkemoney.Checked = true;
                    }
                    else
                    {
                        chkemoney.Checked = false;
                    }
                    txtremschedule.Text = dt.Rows[0]["ApproRemSchedul"].ToString();
                    txtautoremsche.Text = dt.Rows[0]["AutoRemSchedule"].ToString();
                    txtexpirydays.Text = dt.Rows[0]["ExpiryDays"].ToString();
                    //AddressType();
        ...
        public void Cleardept()
        {
           ...
           // TextBox1.Text = "";
            ...
            radiolocationYes.Checked = false;
            txtelapseTime.Text = "";
            txtPreorderTime.Text = "";
            txtecomReq.Text = "";
            hdnRequestorID.Value = "";
            lblmerchantCode.Text = "";
            radiocustanalycsno.Checked = true;
            radioCustanalyticsyes.Checked = false;
            txtexpirydays.Text = "";
        ...
        private void GetCompanyByID()
        {
            ...
            try
            {
               ...
               // ddlCountry.SelectedItem.Text = hdnCountry.Value.ToString();
                ...
                txtsourceEmail.Text = AddList.SourceEmail;
        ...
        {
            CustomProfile profile = CustomProfile.GetProfile();
            AccountSearchService.iCustomerClient Cclient = new AccountSearchService.iCustomerClient();
