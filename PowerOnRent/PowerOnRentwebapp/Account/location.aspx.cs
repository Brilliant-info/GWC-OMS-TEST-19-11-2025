    ...
    public partial class location : System.Web.UI.Page
    {
        ...
        protected void pageSave(Object sender, ToolbarService.iUCToolbarClient e)
        {
            ...
            try
            {
                    ...
                    string address1 = CheckString(txtCAddress1.Text.Trim());
                    string address2 = CheckString(txtAddress2.Text.Trim());
                    ...
                    string Locationname = CheckString(txtlocationName.Text.Trim());
                    ...
                    {
                        DataSet ds = new DataSet();
                        ds = CompanyClient.CheckLocationCode(txtlocation.Text, profile.DBConnection._constr);
                        ...
                        {
                            WebMsgBox.MsgBox.Show("LocationCode Already Exist!");
                        }
                    }
                    else
                    {
        ...
        private void loadstring()
        {
            ...
            {
                Login.Profile.ErrorHandling(ex, this, "Location", "loadstring");
            }
        }

        [WebMethod]
        public static int WMCheckLocationCode(string LCode)
        {
            int dupicate = 0;
            CustomProfile profile = CustomProfile.GetProfile();
            CompanySetupService.iCompanySetupClient CompanyClient = new CompanySetupService.iCompanySetupClient();
            DataSet ds = new DataSet();
            ds = CompanyClient.CheckLocationCode(LCode, profile.DBConnection._constr);
            if (ds.Tables[0].Rows.Count > 0)
            {
                dupicate = 1;
               // WebMsgBox.MsgBox.Show("Location Code Already Exist!");
            }
            return dupicate;
