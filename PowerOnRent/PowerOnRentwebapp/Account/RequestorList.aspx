<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/CRM2.Master" AutoEventWireup="true" CodeBehind="RequestorList.aspx.cs"
    Inherits="PowerOnRentwebapp.Account.RequestorList" Theme="Blue" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function selectedRec() {
            var hdnRequestorName = window.opener.document.getElementById("hdnRequestorName");
            var hdnRequestorID = window.opener.document.getElementById("hdnRequestorID");
            hdnRequestorName.value = "";
            hdnRequestorID.value = "";
            if (GridProductSearch.SelectedRecords.length > 0) {
                for (var i = 0; i < 1; i++) {
                    var record = GridProductSearch.SelectedRecords[i];
                    if (hdnRequestorName.value != "") hdnRequestorName.value += ',' + record.ID;
                    if (hdnRequestorName.value == "") {
                        hdnRequestorName.value = record.AName;
                        hdnRequestorID.value = record.ID;
                    }
                }

                window.opener.AfterRequestorSelect(hdnRequestorName.value, hdnRequestorID.value);
                self.close();
            }
            if (GridProductSearch.SelectedRecords.length == 0) {
