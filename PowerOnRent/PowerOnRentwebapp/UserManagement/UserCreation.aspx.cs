...
namespace PowerOnRentwebapp.UserManagement
{
    ...
    public partial class UserCreation : System.Web.UI.Page
    {
        ...
        protected void pageAddNew(Object sender, ToolbarService.iUCToolbarClient e)
        {
            ...
            Gvassigndept.DataSource = null;
            Gvassigndept.DataBind();
            ...
            CustomProfile profile = CustomProfile.GetProfile();
            userClient.DeleteUserTeritoryifUserIdZero(profile.Personal.UserID.ToString(), profile.DBConnection._constr);
        ...
        protected void pageSave(Object sender, ToolbarService.iUCToolbarClient e)
        {
            ...
            try
            {
                ...
                {
                    MsgBox.Show("Please Add Department to User");
                }
               // GetPrdLst
                // ActiveTab("List");
        ...
        private void GetUserByID()
        {
            ...
            try
            {
                ...
                {
                    string Password = "";
                    string PreCode = dt.Rows[0]["Password"].ToString().Substring(0, 3);
                    if(PreCode == "|@|")
                    {
                        string EncriptwithoutPrecode = dt.Rows[0]["Password"].ToString().Substring(3);
                    ...
                    hdncurrentpassword.Value = Password;         // dt.Rows[0]["Password"].ToString();
                    txtPassword1.Text = Password;                // dt.Rows[0]["Password"].ToString();
                    txtConfirmPassword.Text = Password;           // dt.Rows[0]["Password"].ToString();
                }
                ...
                //AddedControl by suraj
                string str = Encoding.UTF8.GetString(objuser.ProfileImg, 0, objuser.ProfileImg.Length);
                if (str != null || str != "")
                {
        ...
        protected void CreateProfile(string LoginName, long userID)
        {
            ...
            profile.DBConnection._constr[0] = "10.228.134.66";
            //profile.DBConnection._constr[0] = "QADO0TOMSDB01";
            profile.DBConnection._constr[1] = "GWCDEEcomm";
            profile.DBConnection._constr[2] = "Password123#";
            profile.DBConnection._constr[3] = "GWCTEST";

            //profile.DBConnection._constr[0] = "SERVER\\SQLEXPRESS";
            //profile.DBConnection._constr[1] = "GWCDETestNew";
            //profile.DBConnection._constr[2] = "Password123#";
        ...
        private void loadstring()
        {
            ...
            pnldepartment.HeaderText = rm.GetString("Department", ci);
            lbldeptlist.Text = rm.GetString("DeptList", ci);
            Button1.Value = rm.GetString("DeptList", ci);
        ...
        [WebMethod]
        public static List<DesignationService.mDesignation> PMGetDesignation(long CompanyID, long DeptID)
        {
            DesignationService.iDesignationMasterClient DesignationService = new DesignationService.iDesignationMasterClient();
            List<DesignationService.mDesignation> DesignationList = new List<DesignationService.mDesignation>();
            try
        ...
        public void SaveForAdmin()
        {
            ...
            else if (chkDuplicate_aspnetmember == false)
            {
                ...
                if (hndstate.Value != "Edit")
                {
                    ...
                    userClient.SavePasswordDetails(ProfileID, txtEmail.Text, txtLoginId1.Text, txtPassword1.Text, profile.Personal.UserName, ipadd, "SaveUser",profile.Personal.UserID, profile.DBConnection._constr);
                }
                else if (hndstate.Value == "Edit")
                {
                    ...
                    //Update Membership EmailID
                    MembershipUser mu = Membership.GetUser(txtLoginId1.Text);//UserName
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (hnduserID.Value != "0")
                        {
                            ...
                            CustomProfile profile = CustomProfile.GetProfile();
                            ds = UserCreationClient.GetUserRoleID(hnduserID.Value, profile.DBConnection._constr);
                            long roleid = Convert.ToInt64(ds.Tables[0].Rows[0]["RoleID"].ToString());
                            if (roleid != Convert.ToInt64(ddlRole.SelectedValue))
                            {
                                DataSet dss = new DataSet();
                                dss = UserCreationClient.GetApprovalLeveldetailsofUser(hnduserID.Value, profile.DBConnection._constr);
                        ...
                        if (result == true)
                        {
                            Response.Write("<script>alert('Please Remove User From Approvel Level')</script>");
                        }
                        else
                        {
                            ...
                            hdnSelectedCompany.Value = objuser.CompanyID.ToString();
                            // objuser.DepartmentID = Convert.ToInt64(hdnSelectedDepartment.Value);
                            DataSet dsgetdept = new DataSet();
                            iProductMasterClient productClient = new iProductMasterClient();
                            dsgetdept = productClient.GetDepartment(Convert.ToInt64(hdnSelectedCompany.Value), profile.DBConnection._constr);
                            hdnSelectedDepartment.Value = dsgetdept.Tables[0].Rows[0]["ID"].ToString();
                            hdnDepartmentName.Value = dsgetdept.Tables[0].Rows[0]["Territory"].ToString();
                            objuser.DepartmentID = Convert.ToInt64(dsgetdept.Tables[0].Rows[0]["ID"].ToString());
                            objuser.EmployeeID = txtEmpNo.Text.Trim();
                            objuser.FirstName = txtFirstName.Text.Trim();
                            objuser.MiddelName = txtMiddleName.Text.Trim();
                            objuser.LastName = txtLastName.Text.Trim();
                            ...
                            // Session["ProfileImg"] = FileUploadProfileImg.FileBytes;
                            // objuser.ProfileImg = (byte[])Session["ProfileImg"];
                            string str1 = Encoding.UTF8.GetString(FileUploadProfileImg.FileBytes, 0, FileUploadProfileImg.FileBytes.Length);
                            if (str1 == null || str1 == "")
                            {
                                objuser.ProfileImg = (byte[])Session["ProfileImg"];
                            }
                            ...
                            if (hndstate.Value != "Edit")
                            {
                                ...
                                CreateProfile(txtLoginId1.Text, Convert.ToInt64(hnduserID.Value));
                                long ProfileID = Convert.ToInt64(hnduserID.Value);
                                string encriptPassword = encryptstring(txtPassword1.Text);
                                encriptPassword = "|@|" + encriptPassword;
                                userClient.SavePasswordDetails(ProfileID, txtEmail.Text, txtLoginId1.Text, encriptPassword, profile.Personal.UserName, ipadd, "SaveUser",profile.Personal.UserID, profile.DBConnection._constr);
                            }
                            else if (hndstate.Value == "Edit" && Approvallst == 0)
                            {
                                ...
                                if (hdncurrentpassword.Value != txtPassword1.Text.Trim())
                                {
                                    ...
                                    string encriptPasswordcheck = encryptstring(txtPassword1.Text);
                                    encriptPasswordcheck = "|@|" + encriptPasswordcheck;


                                    ds = userClient.CheckPasswordHistory(Convert.ToInt64(hnduserID.Value), encriptPasswordcheck, profile.DBConnection._constr);
                                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                                    {
                                       ...
                                       // string CypherPassword = ds.Tables[0].Rows[i]["Password"].ToString().Substring(3);
                                      //  string DecriptPassword = Decryptstring(CypherPassword);



                                        if (Password == txtPassword1.Text)
                                        {
                                            str = "PasswordFound";
                                        }
                                    }
                                    ...
                                    if (str != "PasswordFound")
                                    {
                                        if (str != "SameDay")
                                        {
                                            ...
                                            if (u.ChangePassword(hdncurrentpassword.Value, txtPassword1.Text.Trim())) {  
                                            //u.ChangePassword(hdncurrentpassword.Value, txtPassword1.Text.Trim());
                                            userClient.SavePasswordDetails(Convert.ToInt64(hnduserID.Value), txtEmail.Text, txtLoginId1.Text, encriptPasswordcheck, profile.Personal.UserName, ipadd, "UpdateUser",profile.Personal.UserID, profile.DBConnection._constr);
                                            SendInstituteMail(txtEmail.Text, txtPassword1.Text);} 
                                            else {
                                                MsgBox.Show("Password Not Changed...");
                                            }
                                        }
                                        else
                                        {
                            ...
                            if (hnduserID.Value != "0" && hnduserID.Value != "")
                            {
                                ...
                                if (hdnSelectedCompany.Value != "")
                                {
                                    ...
                                    userClient.UpdateUserTerritoryDetails(Convert.ToInt64(hnduserID.Value), profile.Personal.UserID.ToString(), profile.DBConnection._constr);
                                    //userClient.SaveUsersLocationDetails(Convert.ToInt64(hnduserID.Value), level, hdnSelectedLocation.Value, profile.Personal.UserID.ToString(), profile.DBConnection._constr);
                                }
                                if (roelSaveResult == true)
                                {
                                    if (hndstate.Value != "Edit")
                                    userClient.RemoveReturnMenuNewUser(Convert.ToInt64(hnduserID.Value), hndstate.Value, profile.DBConnection._constr);
                                    MsgBox.Show("Record save successfully");
                                }
        ...
        protected void RebindDeptGrid(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = new DataSet();
                ds = BinduserDepartment();
        ...
        DataSet BinduserDepartment()
        {
            ...
            if (hndstate.Value == "Edit")
            {
                str = "select UT.ID, T.Territory,T.StoreCode,T.AutoCancel,case when T.ECommerce = 0 then 'No' else 'Yes' end ECommerce, case when T.PriceChange = 0 then 'No' else 'Yes' end PriceChange,T.Active from mTerritory T left outer join mUserTerritoryDetail UT on T.ID = UT.TerritoryID where UT.UserID = '" + long.Parse(hnduserID.Value) + "'";
                SqlDataAdapter da = new System.Data.SqlClient.SqlDataAdapter(str, conn);
                ds1.Reset();
                da.Fill(ds1);
            }
            ...
            {
                str = "select UT.ID, T.Territory,T.StoreCode,T.AutoCancel,case when T.ECommerce = 0 then 'No' else 'Yes' end ECommerce, case when T.PriceChange = 0 then 'No' else 'Yes' end PriceChange, T.Active from mTerritory T left outer join mUserTerritoryDetail UT on T.ID = UT.TerritoryID where UT.UserID = 0 and UT.CreatedBy = '" + CreatedBy + "'";
                SqlDataAdapter da = new System.Data.SqlClient.SqlDataAdapter(str, conn);
                ds1.Reset();
                da.Fill(ds1);
            }
        ...
        [WebMethod]
        public static string RemoveDepartment(object objReq)
        {
            string result = "";
            iUserCreationClient userClient = new iUserCreationClient();
            CustomProfile profile = CustomProfile.GetProfile();
            Dictionary<string, object> dictionary = new Dictionary<string, object>();
            dictionary = (Dictionary<string, object>)objReq;
            long UserDeptID = long.Parse(dictionary["UserDeptID"].ToString());
            userClient.RemoveDepartment(UserDeptID, profile.DBConnection._constr);
            result = "success";
            return result;
        }
        ...
        [WebMethod]
        public static int CheckRole(string userid, long Newroleid, string hndstate)
        {
            int result = 0;
            if (hndstate == "Edit")
            {
                ...
                iUserCreationClient UserCreationClient = new iUserCreationClient();
                CustomProfile profile = CustomProfile.GetProfile();
                ds = UserCreationClient.GetUserRoleID(userid, profile.DBConnection._constr);
                long roleid = Convert.ToInt64(ds.Tables[0].Rows[0]["RoleID"].ToString());
                if (roleid != Newroleid)
                {
                    DataSet dss = new DataSet();
                    dss = UserCreationClient.GetApprovalLeveldetailsofUser(userid, profile.DBConnection._constr);
                    if (dss.Tables[0].Rows.Count > 0)
                    {
                        result = 1;
        ...
        #region Clear Security Question

        [WebMethod]
        public static string WMClearSecurityQuestions (string uid)
        {
            string result = "";
            try
            {
                ...
                {
                    result=UserCreationClient.ClearSecurityQuestions(userid[i], ipadd,"ClearSecurityQuestions", profile.Personal.UserName,profile.Personal.UserID, profile.DBConnection._constr);
                }               
            ...
            {
                result = "failed";
                Login.Profile.ErrorHandling(ex, "Usercreation.aspx", "WMClearSecurityQuestions");
            }
