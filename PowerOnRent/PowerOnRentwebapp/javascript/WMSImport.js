...
function loadTemplateData() {
    ...
    jQuery.ajax({
        ...
        //  url: "http://151.106.38.138/ImportService/DImportService.svc/GetTemplate",
        //  url: "http://localhost:2318/DImportService.svc/GetTemplate",
        // url: "http://164.52.208.105/ImportService/DImportService.svc/GetTemplate",
        //  url: " http://localhost:2318/DImportService.svc/GetTemplate",
        //  url: "http://164.52.208.105/ImportServiceprod/DImportService.svc/GetTemplate",
        url: "http://localhost:2318/DImportService.svc/GetTemplate",
        //url: "https://testoms.gwclogistics.com/ImportService/DImportService.svc/GetTemplate",
...
function loadDataTable() {
    ...
    for (var line = startLine; line < (lines.length - startLine); line++) {
        ...
        if (lines[line] != "") {
            ...
            for (var thc = 0; thc < templateHeaderCount; thc++) {
                var tCellNo = thc + 1;
                strLines += '<td id="l' + rRowNo + 't' + tCellNo + '" class="WMSImportCell">' + csvContent[userColumnSequence[thc]] + '</td>';
            }
            strLines += '</tr>';
            $('#WMSImportTable').append(strLines);
        }
...
function showProgress() {
    ...
    if (currentUploadProgress < totalRecordToUpload) {
        ...
        if (!hasErrorDataOnly) {
            ...
            $('#WMSImportTable .WMSImportRow a').remove();
            // alert('Removed!!');
            goToStep(stepValidateFile);
            showLastAsActiveTab();
        } else {
...
function sendDataToService() {
    ...
    if (extractCell != '') {
        ...
        var dataParentObject = {
            "ImportDetails": dataObject
        }
        jQuery.ajax({
            type: "POST",
            // url: "http://localhost:2318/DImportService.svc/ImportRequst",
            // url: "http://localhost/WMSImportService/DImportService.svc/ImportRequst",
            //   url: "https://w5production.westcoastwarehouse.com/ImportService/DImportService.svc/ImportRequst",
            //  url: "http://151.106.38.138/ImportService/DImportService.svc/ImportRequst",

            //  url: "http://localhost:2318/DImportService.svc/ImportRequst",

            // url: "http://164.52.208.105/ImportService/DImportService.svc/ImportRequst",
            // url: "http://164.52.208.105/ImportServiceprod/DImportService.svc/ImportRequst",
            // url: "http://164.52.208.105/ImportServiceprod/DImportService.svc/ImportRequst",
            url: "http://localhost:2318/DImportService.svc/ImportRequst",
          //  url: "https://testoms.gwclogistics.com/ImportService/DImportService.svc/ImportRequst",
...
function serverResponseReader(response, startUploadRowNum, endUploadRowNum) {
    ...
    var serverResult = response["ImportRequstResult"]["Description"];
    var serverStatusCode = response["ImportRequstResult"]["StatusCode"];
    var serverFailRows = response["ImportRequstResult"]["FailRows"];
...
function showFiles(files) {
    ...
    if (files[0]) {
        ...
        if ((currentFileType == 'CSV') && (getExtension == '.csv')) {
            isValidFile = true;
        } else if ((currentFileType == 'Text') && ((getExtension == '.txt') || (getBigExtension == '.text'))) {
            generateTextPreview();
            isValidFile = true;
        } else if ((currentFileType == 'Excel') && ((getExtension == '.xls') || (getBigExtension == '.xlsx'))) {
            isValidFile = true;
        } else {
    ...
    if (CustomerId != '0') {
        if (templateId != '0') {
          ...
          //  window.location.href = "DownloadTemplate.ashx?tid=" + templateId + "&type=" + tmpltype + "&ObjectName=" + objectName;
            ...
            window.location.href = "ImportPrice.xlsx";
        }
        else
        {
            alert('Error to load template details for selected customer!!');
...
function FinishImport() {
    var getImportObjectName = $('#hdnImportObject').val();
    var url = "";
    ...
    else if (getImportObjectName == 'PurchaseOrderGrn') {
        window.location.href = '../WMS/GridGRN.aspx?POID=0';
    }
    else if (getImportObjectName == 'SalesReturnGrn') {
        window.location.href = '../WMS/GridGRN.aspx?POID=0';
    }
    else if (getImportObjectName == 'Inventory') {
        window.location.href = '../Product / ProductInventory.aspx';
    }
