...
namespace PowerOnRentwebapp.Mobile
{
    ...
    public class get_products_by_orderid : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            long reId = Convert.ToInt64(context.Request.QueryString["reId"]);

            cmd.CommandType = CommandType.Text;
            cmd.CommandText = " select OD.SkuId,OD.Prod_Name,OD.Prod_Description,OD.Prod_Code,OD.OrderQty,um.Description UOM,IM.Path,Mp.GroupSet  from tOrderDetail OD left outer join tImage IM on OD.SkuId=IM.ReferenceID left outer join muom um on OD.UOMID=um.ID left outer join mProduct MP on OD.SkuId=MP.ID  where OD.OrderHeadId=" + reId + "";            
            cmd.Connection = conn;
            ...
            if(cntr>0)
            {
                for (int i = 0; i < cntr; i++)
                {
                    ...
                    xmlString = xmlString + "<prodName>" + Prod_Name + "</prodName>";
                    xmlString = xmlString + "<prodDescription>" + Prod_Description + "</prodDescription>";
