...
namespace PowerOnRentwebapp.Mobile
{
    ...
    public class get_products_by_templateid : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            cmd.CommandText = " select TD.TemplateHeadID,TD.PrdId,TD.UOMID,TD.Qty,  MP.ProductCode,MP.Name,case when MP.OMSSKUCode is NULL then 'N/A' else MP.OMSSKUCode  End  OMSSKUCode,psd.AvailableBalance,im.Path,MP.GroupSet,um.Description UOM from mRequestTemplateDetail TD  left outer join mProduct MP on TD.PrdId=MP.ID left outer join muom um on TD.UOMID=um.ID left outer join timage im on TD.PrdId=im.ReferenceID left outer join tproductstockdetails psd on TD.PrdId=psd.ProdID  where TD.TemplateHeadID=" + tmplId + "";
            cmd.Connection = conn;
            ...
            if (cntr > 0)
            {
                for (int i = 0; i < cntr; i++)
                {
                    ...
                    xmlString = xmlString + "<productTitle>" + Prod_Name + "</productTitle>";
