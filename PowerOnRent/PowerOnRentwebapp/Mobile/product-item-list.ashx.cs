...
namespace PowerOnRentwebapp.Mobile
{
    ...
    public class product_item_list1 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            cmd.CommandText = "select MP.ID,MP.ProductCode,MP.Name,case when MP.OMSSKUCode is NULL then 'N/A' else MP.OMSSKUCode  End  OMSSKUCode,psd.AvailableBalance,im.Path from mproduct MP left outer join tproductstockdetails psd on MP.ID=psd.ProdID left outer join timage im on MP.ID=im.ReferenceID ";
            cmd.Connection = conn;
            ...
            for (int i = 0; i < cntr; i++)
            {
                ...
                xmlString = xmlString + "<itemTitle>Request No. " + itemTitle + "</itemTitle>";
