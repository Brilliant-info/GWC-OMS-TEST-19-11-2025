...
namespace PowerOnRentwebapp.Mobile
{
    ...
    public class get_product_billofmaterial : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            String myQueryString = "select BD.SKUId,MP.ProductCode,MP.Name,MP.Description,BD.Quantity,img.Path from mBOMDetail BD left outer join mProduct MP on BD.SKUId=MP.ID left outer join tImage img on  BD.SKUId=img.ReferenceID where BD.BOMheaderId=" + prdId + "";
            ...
            if (cntr > 0)
            {
                for (int i = 0; i < cntr; i++)
                {
                    ...
                    xmlString = xmlString + "<productName>" + Name + "</productName>";
                    xmlString = xmlString + "<productDescription>" + Description + "</productDescription>";
