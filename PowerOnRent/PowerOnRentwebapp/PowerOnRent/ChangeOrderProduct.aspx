<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/CRM2.Master" AutoEventWireup="true" CodeBehind="ChangeOrderProduct.aspx.cs"
    Inherits="PowerOnRentwebapp.PowerOnRent.ChangeOrderProduct" EnableEventValidation="false" Theme="Blue" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
            ...
            <ContentTemplate>
                <table class="tableForm">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" id="txtPrc" runat="server" onblur="GetSelValue()" onkeydown="AllDec(this,event); GetSelValue();"
                                onkeypress="AllDec(this,event); GetSelValue();" />
    ...
    <asp:HiddenField ID="hdnqty" runat="server" ClientIDMode="Static" />
    <asp:HiddenField ID="hdnQuantity" runat="server" ClientIDMode="Static" />
    <asp:HiddenField ID="hdnSelectedOrdId" runat="server" ClientIDMode="Static" />
    ...
    <script type="text/javascript">
        function GetIndexQty() {
            ...
            var qty = document.getElementById('hdnqty');
            UOMQTY = document.getElementById("<%= hdnQuantity.ClientID %>");

            var numTotalQty = Number(UOMQTY.value) * Number(hdnEnteredQty.value);  //var numTotalQty = Number(UOMQTY) * Number(EnteredValue);
        ...
        function GetSelValue() {
            ...
            var numTotalQty = Number(UOMQTY.value) * Number(EnteredValue);
            var txtOrderQty = document.getElementById("<%= txtOrderQty.ClientID %>");
            txtOrderQty.value = numTotalQty;
            var txtPrice = document.getElementById("<%= txtPrc.ClientID %>");
            var Price = txtPrice.value;
        ...
        function GetUpdatedDetails() {
            ...
            //commit for wrong msg show 03-01-2018
            // else if (TTL<=0){//(txtPrice.value == "0" || txtPrice.value == "0.00" || txtPrice.value == "00") {
            //     showAlert("Price should not be Zero...!!!", "Error", '#');
            // }  
            else if (OrderQty <= 0) {
        ...
        function OnSuccessUpdate(result) {
            if (result == 1) {
                ...
                if (r == true) {
                    var hdnSelectedProduct = document.getElementById("<%= hdnSelectedProduct.ClientID %>");
                    var hdnSelectedOrdId = document.getElementById("<%= hdnSelectedOrdId.ClientID %>");
                    var txtOrderQty = document.getElementById("<%= txtOrderQty.ClientID %>"); var OrderQty = txtOrderQty.value;
                    var txtPrice = document.getElementById("<%= txtPrc.ClientID %>"); var Price = txtPrice.value;
                    var txtTotal = document.getElementById("<%= txtTotal.ClientID %>"); var Total = txtTotal.value;

                    PageMethods.WMRemoveAssignSkuSerial(hdnSelectedOrdId.value, hdnSelectedProduct.value, result, OrderQty, Price, Total, OnSuccessRemoveAssignSkuSerial, null);
                    //var hdnChangePrdQtyPrice = window.opener.document.getElementById("hdnChangePrdQtyPrice");
                    //hdnChangePrdQtyPrice.value = "1";
                    //window.opener.AfterProductQtyChange(hdnChangePrdQtyPrice.value);
                    //self.close();
