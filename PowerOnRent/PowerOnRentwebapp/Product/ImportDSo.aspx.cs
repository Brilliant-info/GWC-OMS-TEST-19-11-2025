...
namespace PowerOnRentwebapp.POR
{
    public partial class ImportDSo : System.Web.UI.Page
    {
        ...
        protected void btnUploadPo_Click(object sender, EventArgs e)
        {
                            ...
                            if (availableProdId != 0)
                            {
                                ...
                                if (getSKUIdfromtImage == 0)
                                {
                                    ...
                                    else
                                    {
                                     ...
                                     //   Error = "Y";
                                        Reason = "Please select file with size less than 60kb.";
                                        destFile = System.IO.Path.Combine(targetPath, fileName);
                                        destFile1 = System.IO.Path.Combine(dbpath, fileName);
            ...
            {
                Login.Profile.ErrorHandling(ex, this, "ImportDSo", "btnUploadPo_Click");
                Array.ForEach(Directory.GetFiles(folderpath1), File.Delete);
                // ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Image Import failed - Please select file with size less than 30kb.');", true);
            }
            finally
            {
                productClient.Close();
